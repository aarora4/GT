...
from proj1_code.models import HybridImageModel, create_2D_Gaussian_kernel
...
def test_dataloader_len():
    ...
    """
    img_dir = f'{ROOT}/data'
    cut_off_file = f'{ROOT}/cutoff_standarddeviations.txt'
...
def test_low_pass_filter_square_kernel():
    ...
    low_pass_filter = create_2D_Gaussian_kernel(standard_deviation=7)
    k_h, k_w = low_pass_filter.shape
    student_filtered_img = my_imfilter(dog_img, low_pass_filter)

    # Exclude the border pixels.
    student_filtered_img_interior = student_filtered_img[k_h:img_h -
                                                         k_h, k_w:img_w-k_w]
...
def test_random_filter_nonsquare_kernel():
    ...
    student_output = my_imfilter(
        torch.from_numpy(image), torch.from_numpy(kernel))
    ...
    student_center_crop = student_output[h_center -
                                         1:h_center+1, w_center-1:w_center+1]
...
def verify_low_freq_sq_kernel_torch_manual(image1, kernel, low_frequencies) -> bool:
    """
    ...
    correct_crop = np.allclose(
        low_frequencies[100:102, 100:102, :], gt_low_freq_crop, atol=1e-3)
...
def verify_high_freq_sq_kernel_torch_manual(image2, kernel, high_frequencies) -> bool:
    """
            Interactive test to be used in IPython notebook, that will print out
            test result, and return value can also be queried for success (true).
    ...
    # H,W,C order in Numpy
    correct_crop = np.allclose(
        high_frequencies[100:102, 100:102, :], gt_high_freq_crop, atol=1e-3)
    if correct_sum and correct_crop:
...
def verify_hybrid_image_torch_manual(image1, image2, kernel, hybrid_image) -> bool:
    """
            Interactive test to be used in IPython notebook, that will print out
            test result, and return value can also be queried for success (true).
...
def verify_gaussian_kernel(kernel, cutoff_sd) -> bool:
    """
    Interactive test to be used in IPython notebook, that will print out
    test result, and return value can also be queried for success (true).
...
def test_pytorch_low_pass_filter_square_kernel():
    ...
    img_dir = f'{ROOT}/data'
    cut_off_file = f'{ROOT}/cutoff_standarddeviations_temp.txt'

    # Dump to a file
    cutoff_sds = [7, 7, 7, 7, 7]
    write_objects_to_file(fpath=cut_off_file, obj_list=cutoff_sds)
    hi_dataset = HybridImageDataset(img_dir, cut_off_file)

    # should be the dog image
    img_a, img_b, cutoff_sd = hi_dataset[0]
    # turn CHW into NCHW
    img_a = img_a.unsqueeze(0)

    hi_model.n_channels = 3
    kernel = hi_model.get_kernel(cutoff_sd)
    ...
    assert torch.allclose(
        pytorch_low_freq[:, :, 20:22, 20:22], gt_crop, atol=1e-3)

    # ground truth element sum
    assert np.allclose(pytorch_low_freq.numpy().sum(), 209926.3481)


def verify_low_freq_sq_kernel_pytorch(image_a, model, cutoff_sd, low_frequencies) -> bool:
    """
            ...
            -	None
    """
    if not isinstance(cutoff_sd, torch.Tensor) or not torch.allclose(cutoff_sd, torch.Tensor([7])):
...
def verify_high_freq_sq_kernel_pytorch(image_b, model, cutoff_sd, high_frequencies) -> bool:
    """
            Test the output of the forward pass.
    ...
    correct_sum = np.allclose(12.012651, float(
        high_freq_interior.sum()), atol=1e-1)
...
def verify_hybrid_image_pytorch(image_a, image_b, model, cutoff_sd, hybrid_image) -> bool:
    """
            Test the output of the forward pass.
