...
def transformation_matrix(wRc_T, wtc):
    ...
    # TODO: YOUR CODE HERE
    ############################################################################
    prod = -1 * wRc_T @ wtc.T # 3 x 1
    res = np.concatenate([wRc_T, prod], 1) # 3 x 4
    M = np.concatenate([res, np.zeros((1, 4))], 0) # 4x4
    M[3, 3] = 1
    #############################################################################
    #                             END OF YOUR CODE
    ############################################################################
...
def convert_3d_points_to_camera_coordinate(M, points_3d_w):
    ...
    # TODO: YOUR CODE HERE
    ############################################################################
    points_3d_w = points_3d_w.T
    if points_3d_w.shape[0] == 3:
        points_3d_w = np.concatenate((points_3d_w, np.ones((1, points_3d_w.shape[1]))), 0)
    points_3d_c = M @ points_3d_w
    points_3d_c = points_3d_c.T
    #############################################################################
    #                             END OF YOUR CODE
    ############################################################################
    return points_3d_c
...
def projection_from_camera_coordinates(K, points_3d_c):
    ...
    ############################################################################
    points_3d_c = points_3d_c.T
    if points_3d_c.shape[0] == 3:
        points_3d_c = np.concatenate((points_3d_c, np.ones((1, points_3d_c.shape[1]))), 0)
    M = np.concatenate([np.eye(3), np.zeros((3, 1))], 1)
    projected_points_2d = K @ M @ points_3d_c
